...
```{r}
DebtTothePenny[1:5,2] # Select the first row, all columns
```
...
```{r}
 DebtTothePenny$Date       <- as.Date(DebtTothePenny$Effective.Date, "%m/%d/%y")
 DebtTothePenny$PublicDebt <- parse_number(DebtTothePenny$Public.Debt)
 DebtTothePenny$TotalDebt  <- parse_number(DebtTothePenny$Total.Debt)
```
...
```{r}
class(DebtTothePenny$Public.Debt)
```
